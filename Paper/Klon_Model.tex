...
thereby allowing the model to have a smaller memory footprint which
can often be important for embedded implementations. The GRU consists
of three ``gates'': the update gate $z[n]$, reset gate $r[n]$, and the
new gate $c[n]$. These gates are used to compute the cell's current
output $h[n]$ from its current input $x[n]$ and previous output
$h[n-1]$ as follows:
\begin{equation}
